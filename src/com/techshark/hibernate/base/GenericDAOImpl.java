...
/* 23: */{
	/* 24: 37 */static final Logger LOG = LoggerFactory
	...
	/* 31: */{
		/* 32: 49 */this.persistentClass = ((Class) ((java.lang.reflect.ParameterizedType) getClass()
	...
	/* 67: */{
		...
		/* 71: */try
		/* 72: */{
			/* 73: 94 */Query query = session.createSQLQuery(sql).addEntity(
					getPersistentClass());
	...
	/* 94: */{
		...
		/* 99: */{
			...
			/* 101:117 */LOG.debug(MODULE + "getExecuteUpdate" + "# Query : "
					+ hql);
	...
	/* 120: */
	/* 121: */public DomainObject getRecordByKeyandValue(String keyName,
			Object keyValue)
	/* 122: */{
		...
		/* 126: */try
		/* 127: */{
			/* 128:145 */Criteria query = session.createCriteria(
					getPersistentClass()).add(
					Restrictions.eq(keyName, keyValue));
		...
		/* 135: */{
			/* 136:151 */LOG.error(MODULE
					+ "Exception in getRecordByKeyandValue Method:" + e, e);
			/* 137: */}
		/* 138: */finally
		/* 139: */{
	...
	/* 146: */
	/* 147: */public List<DomainObject> getListByKeyandValue(String colname,
			Object value)
	/* 148: */{
		...
		/* 151: */try
		/* 152: */{
			/* 153:171 */Criteria query = session.createCriteria(
					getPersistentClass()).add(Restrictions.eq(colname, value));
	...
	/* 169: */
	/* 170: */public List<DomainObject> getListByKeyandValue(String colname,
	...
	/* 171: */{
		...
		/* 174: */try
		/* 175: */{
			/* 176:194 */Criteria query = session.createCriteria(
					getPersistentClass()).add(
					Restrictions.in(colname, keyValue));
	...
	/* 194: */{
		...
		/* 198: */{
			/* 199:216 */if ((condition == null) || ("null".equals(condition))) {
				/* 200:217 */condition = " ";
				/* 201: */}
			/* 202:219 */lst = session.createQuery(
					"from " + getPersistentClass().getName() + " " + condition)
					.list();
			/* 203: */}
		/* 204: */catch (HibernateException e)
		/* 205: */{
			/* 206:221 */LOG.error(MODULE
					+ "Exception in getListByCondition Method:" + e, e);
			/* 207: */}
		/* 208: */finally
		/* 209: */{
	...
	/* 218: */{
		...
		/* 220: */try
		/* 221: */{
			/* 222:240 */return (DomainObject) session.get(
					getPersistentClass(), id);
			/* 223: */}
		/* 224: */catch (HibernateException e)
		/* 225: */{
			/* 226:242 */LOG.error(MODULE
					+ "Exception in getRecordByPrimaryKey Method:" + e, e);
			/* 227:243 */throw e;
			/* 228: */}
		/* 229: */finally
		/* 230: */{
	...
	/* 238: */{
		...
		/* 242: */{
			/* 243:261 */if ((condition == null) || ("null".equals(condition))) {
				/* 244:262 */condition = " ";
				/* 245: */}
			/* 246:264 */ClassMetadata cm = this.sessionFactory
					.getClassMetadata(getPersistentClass().getName());
			/* 247:265 */String type = cm.getIdentifierPropertyName();
			/* 248:266 */lst = session.createQuery(
					"select t." + type + " from "
							+ getPersistentClass().getName() + " t" + " "
							+ condition).list();
			/* 249: */}
	...
	/* 265: */{
		...
		/* 271: */{
			...
			System.out
					.println("OBJECT IS SAVED SUCCESSFULLY>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");

			/* 275: */}
		/* 276: */catch (RuntimeException e)
		/* 277: */{
			/* 278:293 */LOG.error(MODULE + "Exception in insert Method:" + e,
					e);
			/* 279:294 */if (tx != null) {
				/* 280:295 */tx.rollback();
				/* 281: */}
	...
	/* 294: */{
		...
		/* 299: */{
			...
			/* 301: */{
				/* 302:319 */LOG.debug(MODULE
						+ " Null object could not update....RETURNING");
		...
		/* 311: */{
			...
			/* 313:328 */LOG.error(MODULE + "Exception in update Method." + e,
					e);
			/* 314:329 */if (tx != null) {
				/* 315:330 */tx.rollback();
				/* 316: */}
	...
	/* 329: */{
		...
		/* 334: */{
			...
			/* 336: */{
				/* 337:352 */LOG.debug(MODULE
						+ " Null object could not delete....RETURNING");
		...
		/* 347: */{
			...
			/* 349:362 */LOG.error(MODULE + "Exception in delete Method." + e,
					e);
			/* 350:363 */if (tx != null) {
				/* 351:364 */tx.rollback();
				/* 352: */}
		...
		/* 355: */{
			...
			/* 357:368 */LOG.error(MODULE + "Exception in delete Method." + e,
					e);
			/* 358:369 */if (tx != null) {
				/* 359:370 */tx.rollback();
				/* 360: */}
	...
	/* 373: */{
		...
		/* 383: */{
			/* 384:395 */LOG.error(MODULE + "Exception in deletion through id."
					+ e, e);
			/* 385:396 */if (tx != null) {
				/* 386:397 */tx.rollback();
				/* 387: */}
	...
	/* 399: */{
		...
		/* 402: */try
		/* 403: */{
			/* 404:416 */count = (Long) session.createQuery(
					"select count(t) from " + getPersistentClass().getName()
							+ " t ").uniqueResult();
			/* 405: */}
	...
	/* 421: */{
		...
		/* 425: */{
			/* 426:438 */if ((condition == null) || ("null".equals(condition))) {
				/* 427:439 */condition = " ";
				/* 428: */}
			/* 429:441 */count = (Long) session.createQuery(
					"select count(t) from " + getPersistentClass().getName()
							+ " t " + condition).uniqueResult();
			/* 430: */}
	...
	/* 497: */
	/* 498: */public List<DomainObject> getListByPage(String columnName,
	...
	/* 499: */{
		...
		/* 502: */try
		/* 503: */{
			/* 504:524 */String query = " from "
					+ getPersistentClass().getName() + " order by "
					+ columnName + " " + sortOrder;
			/* 505:525 */lst = session.createQuery(query).setMaxResults(limit)
	...
	/* 523: */{
		...
		/* 526: */try
		/* 527: */{
			/* 528:546 */lst = session.createQuery(query).setMaxResults(limit)
					.setFirstResult(offset).list();
			/* 529: */}
	...
	/* 544: */
	/* 545: */public String getJsonPageData(int pageNo, int offset,
			String sortColumn, String sortOrder)
	/* 546: */{
		/* 547:564 */LOG.debug(MODULE
				+ "Reached here for getting json response.");
		/* 548: */
		/* 549: */
		/* 550:567 */int beginIndex = (pageNo - 1) * offset;
		/* 551:568 */int totalRecods = count();
		/* 552:569 */int totalPages = totalRecods / offset;
		/* 553:570 */LOG.debug(MODULE + "pageNo:" + pageNo + ":" + totalRecods
				+ ":" + totalPages + ":" + totalRecods % offset);
		/* 554:572 */if (totalPages == 0) {
			/* 555:573 */totalPages = 1;
			/* 556: */} else {
			/* 557:575 */totalPages = totalRecods / offset
					+ (totalRecods % offset != 0 ? 1 : 0);
			/* 558: */}
		...
		/* 563: */
		/* 564:583 */List<DomainObject> listByPage = getListByPage(sortColumn,
				sortOrder, offset, beginIndex);
		/* 565:584 */int totalSize = listByPage != null ? listByPage.size() : 0;
		/* 566: */
		/* 567:586 */LOG.debug(MODULE + "TOTAL RECORDS : "
				+ (offset > totalSize ? totalSize : offset));
		/* 568: */
		/* 569: */
		/* 570: */
	...
	/* 574: */
	/* 575: */public String getJson(int pageNo, int offset, String sortColumn,
			String sortOrder, List<DomainObject> list)
	/* 576: */{
		/* 577:596 */LOG.debug(MODULE
				+ "Reached here for getting json response.");
		/* 578: */
		/* 579: */
		...
		/* 583:602 */LOG.debug(MODULE + "pageNo:" + pageNo + ":" + totalRecods
				+ ":" + totalPages + ":" + totalRecods % offset);
		/* 584:604 */if (totalPages == 0) {
			/* 585:605 */totalPages = 1;
			/* 586: */} else {
			/* 587:607 */totalPages = totalRecods / offset
					+ (totalRecods % offset != 0 ? 1 : 0);
			/* 588: */}
		...
		/* 596: */
		/* 597:618 */LOG.debug(MODULE + "TOTAL RECORDS : "
				+ (offset > totalSize ? totalSize : offset));
		/* 598: */
		/* 599: */
		/* 600: */
	...
	/* 604: */
	/* 605: */public List getListByHQLQuery(String hql, int totalrows,
			int firstrow)
	/* 606: */{
		...
		/* 609: */try
		/* 610: */{
			/* 611:636 */lst = session.createQuery(hql)
					.setMaxResults(totalrows).setFirstResult(firstrow).list();
			/* 612: */}
	...
	/* 666: */
	/* 667: */public List getRecordListByKeyandValue(String keyName,
			Object keyValue)
	/* 668: */{
		...
		/* 672: */try
		/* 673: */{
			/* 674:699 */Criteria query = session.createCriteria(
					getPersistentClass()).add(
					Restrictions.eq(keyName, keyValue));
	...
	/* 704: */{
		...
		/* 714: */{
			/* 715:730 */LOG.error(MODULE + "Exception in insert Method:" + e,
					e);
			/* 716:731 */if (tx != null) {
				/* 717:732 */tx.rollback();
				/* 718: */}
