...
/* 37: */{
	...
	/* 49: */public ModelAndView leaveTypes(
	...
	/* 51: */{
		...
		/* 54: 63 */List<EmployeeLeaveTypes> leaveTypes = this.employeeLeaveTypesDAO
				.getList();
		/* 55: */
	...
	/* 67: */public ModelAndView addLeaveTypeGet(
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error,
			Locale locale)
	/* 68: */throws Exception
	...
	/* 84: */public ModelAndView addLeaveTypePost(
			@ModelAttribute("leaveType") @Valid EmployeeLeaveTypes employeeLeaveTypes,
	...
	/* 86: */{
		...
		/* 89: */{
			/* 90: 99 */int insertResult = ((Integer) this.employeeLeaveTypesDAO
					.insert(employeeLeaveTypes)).intValue();
			/* 91:100 */if (insertResult > 0)
			/* 92: */{
				/* 93:101 */logger
						.info("Employee leave types Added Successfully with id "
								+ insertResult);
				/* 94:102 */return new ModelAndView(
						"redirect:add_leave_type.html").addObject("success",
						Utilities.getSpringMessage(this.messageSource,
								"leavetype.add.success", locale));
				/* 95: */}
			/* 96:105 */logger.info("Error while inserting "
					+ employeeLeaveTypes);
			/* 97:106 */return new ModelAndView("add_leave_type", model)
					.addObject("error", Utilities.getSpringMessage(
							this.messageSource, "leavetype.add.error", locale));
			/* 98: */}
	...
	/* 103: */public ModelAndView editLeaveTypeGet(@RequestParam("id") int id,
			Locale locale,
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error)
	...
	/* 105: */{
		...
		/* 107: */
		/* 108:122 */EmployeeLeaveTypes employeeLeaveTypes = (EmployeeLeaveTypes) this.employeeLeaveTypesDAO
		...
		/* 116: */{
			...
			/* 118:129 */List<EmployeeLeaveTypes> leaveTypes = this.employeeLeaveTypesDAO
					.getList();
			/* 119: */
	...
	/* 132: */public ModelAndView editLeaveTypePost(
			@ModelAttribute("leaveType") @Valid EmployeeLeaveTypes employeeLeaveTypes,
			BindingResult result, ModelMap model, Locale locale)
	...
	/* 134: */{
		...
		/* 137: */{
			/* 138:149 */int updateResult = this.employeeLeaveTypesDAO
					.update(employeeLeaveTypes);
			/* 139:150 */if (updateResult > 0)
			/* 140: */{
				/* 141:151 */logger.info(
						"leave_type updated Successfully with id={0}",
						Integer.valueOf(updateResult));
				/* 142:152 */ModelAndView mav = new ModelAndView(
						"redirect:leave_types.html");
				/* 143:153 */List<EmployeeLeaveTypes> leaveTypes = this.employeeLeaveTypesDAO
						.getList();
				/* 144:154 */mav.getModelMap()
						.put("leaveTypesList", leaveTypes);
				/* 145:155 */return mav.addObject("success", Utilities
						.getSpringMessage(this.messageSource,
								"leavetype.update.success", locale));
				/* 146: */}
			/* 147:158 */logger.warn("Error occurred updating leave type :{0}",
					employeeLeaveTypes);
			/* 148:159 */return new ModelAndView("edit_leave_type", model)
					.addObject("error", Utilities.getSpringMessage(
							this.messageSource, "leavetype.update.error",
							locale));
			/* 149: */}
		/* 150:163 */logger.warn("Leave types values are not valid:",
				employeeLeaveTypes);
	...
	/* 155: */@ResponseBody
	/* 156: */public ModelAndView deleteLeaveType(@RequestParam("id") int id,
			Locale locale)
	/* 157: */throws Exception
	/* 158: */{
		/* 159:171 */logger.info("deleteLeaveType called.");
		/* 160:172 */EmployeeLeaveTypes employeeLeaveTypes = (EmployeeLeaveTypes) this.employeeLeaveTypesDAO
				.getRecordByPrimaryKey(Integer.valueOf(id));
		/* 161:173 */ModelAndView mav = new ModelAndView(
				"redirect:leave_types.html");
		/* 162:174 */List<EmployeeLeaveTypes> leaveTypes = this.employeeLeaveTypesDAO
				.getList();
		/* 163:175 */mav.getModelMap().put("leaveTypesList", leaveTypes);
		/* 164:176 */if (employeeLeaveTypes != null)
		/* 165: */{
			/* 166:177 */int updateResult = this.employeeLeaveTypesDAO
					.delete(employeeLeaveTypes);
			/* 167:179 */if (updateResult > 0)
			/* 168: */{
				/* 169:180 */logger.info(
						"Leave Type with id {0} deleted successfully",
						employeeLeaveTypes.getId());
				/* 170: */
				/* 171:182 */return mav.addObject("success", Utilities
						.getSpringMessage(this.messageSource,
								"leavetype.delete.success", locale));
				/* 172: */}
			/* 173:185 */logger.warn(
					"Error occurred while deleting leave type with id {0}",
					employeeLeaveTypes.getId());
			/* 174:186 */return mav.addObject("error", Utilities
					.getSpringMessage(this.messageSource,
							"leavetype.delete.error", locale));
			/* 175: */}
		/* 176:190 */logger.info("leave type with id {0} is already deleted",
				employeeLeaveTypes.getId());
		/* 177:191 */return mav.addObject("success", Utilities
				.getSpringMessage(this.messageSource,
						"leavetype.delete.success", locale));
		/* 178: */}
	...
	/* 181: */public ModelAndView attendanceRegister(
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error,
			Locale locale)
	...
	/* 183: */{
		...
		/* 186:201 */mav.getModelMap().put("attendance",
				new EmployeeAttendances());
	...
	/* 197: */public ModelAndView addAttendance(
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error,
			Locale locale)
	...
	/* 199: */{
		...
		/* 203:218 */List<EmployeeLeaveTypes> employeeLeaveTypeList = this.employeeLeaveTypesDAO
				.getList();
	...
	/* 228: */public ModelAndView addAttendance(
			@ModelAttribute("attendance") @Valid EmployeeAttendances employeeAttendances,
			BindingResult result, ModelMap model, Locale locale)
	...
	/* 230: */{
		...
		/* 233: */{
			/* 234:249 */List<Employees> employeeList = this.employeesDAO
					.getList();
			/* 235:250 */List<EmployeeLeaveTypes> employeeLeaveTypeList = this.employeeLeaveTypesDAO
					.getList();
			/* 236: */
			...
			/* 248:263 */mav.getModelMap().put("employeeLeaveTypeList",
					employeeLeaveTypeList);
			/* 249:264 */mav.getModelMap().put("hoursList", hoursList);
			/* 250:265 */mav.getModelMap().put("minutesList", minutesList);
			/* 251:266 */mav.getModelMap().put("attendance",
					new EmployeeAttendances());
			/* 252:267 */employeeAttendances.setInTime(employeeAttendances
					.getInHour() + ":" + employeeAttendances.getInMinutes());
			/* 253:268 */employeeAttendances.setOutTime(employeeAttendances
					.getOutHour() + ":" + employeeAttendances.getOutMinutes());
			/* 254:269 */if (!employeeAttendances.getIsLeave().booleanValue()) {
				/* 255:271 */employeeAttendances.setEmployeeLeaveTypeId(null);
				/* 256: */}
			/* 257:273 */int insertResult = ((Integer) this.employeeAttendancesDAO
					.insert(employeeAttendances)).intValue();
			/* 258:274 */if (insertResult > 0)
			/* 259: */{
				/* 260:275 */logger
						.info("Employee attendance Added Successfully with id "
								+ insertResult);
				/* 261:276 */return mav.addObject("success", Utilities
						.getSpringMessage(this.messageSource,
								"attendance.add.success", locale));
				/* 262: */}
			/* 263:279 */logger.info("Error while inserting "
					+ employeeAttendances);
			/* 264:280 */return mav.addObject("error", Utilities
					.getSpringMessage(this.messageSource,
							"attendance.add.error", locale));
			/* 265: */}
	...
	/* 270: */public ModelAndView getAttendanceRegister(
			@ModelAttribute("attendance") @Valid EmployeeAttendances employeeAttendances,
			BindingResult result, ModelMap model, Locale locale)
	...
	/* 272: */{
		...
		/* 275: */{
			/* 276:296 */List<Employees> employeeList = this.employeesDAO
					.getList();
			/* 277:297 */Map<Integer, String> employeeMap = new LinkedHashMap();
			/* 278:298 */for (Employees employees : employeeList) {
				/* 279:299 */employeeMap.put(employees.getId(),
						employees.getEmployeeNumber());
				/* 280: */}
			/* 281:301 */List<EmployeeAttendances> employeeAttendanceList = getEmployeeAttendanceByDate(employeeAttendances
					.getAttendanceDate());
			/* 282: */
			/* 283:303 */ModelAndView mav = new ModelAndView("add_attendance");
			/* 284:304 */mav.getModelMap().put("employeeMap", employeeMap);
			/* 285:305 */mav.getModelMap().put("employeeAttendanceList",
					employeeAttendanceList);
			/* 286:306 */mav.getModelMap().put("attendance",
					new EmployeeAttendances());
			/* 287:307 */return mav;
			/* 288: */}
	...
	/* 291: */
	/* 292: */private List<EmployeeAttendances> getEmployeeAttendanceByDate(
			Date attendanceDate)
	/* 293: */{
		/* 294:313 */return this.employeeAttendancesDAO
				.getListByFromClause(" FROM EmployeeAttendances e WHERE e.attendanceDate="
						+ attendanceDate);
		/* 295: */}
	...
	/* 355: */{
		/* 356:374 */return this.employeeAttendancesDAO
				.getEmployeeAttendanceBetweenDates(fromDate, toDate);
		/* 357: */}

	/* 358: */
	/* 359: */private void setAttendanceData(AttendanceDate attendanceDate,
			Calendar cal, EmployeeAttendances employeeAttendances)
	/* 360: */{
		/* 361:378 */int dayOfWeek = cal.get(7);
		/* 362:379 */logger.info("setAttendanceData called " + dayOfWeek
				+ "   " + 7 + "  " + 1);
		/* 363: */
		/* 364:381 */logger.info(" 111 setAttendanceData called");
		/* 365:382 */attendanceDate.setTotalDays(Integer.valueOf(attendanceDate
				.getTotalDays().intValue() + 1));
		/* 366:383 */if (employeeAttendances.getIsLeave().booleanValue())
		/* 367: */{
			/* 368:384 */logger.info("isleave");
			/* 369:385 */attendanceDate.setAbsentDays(Integer
					.valueOf(attendanceDate.getAbsentDays().intValue() + 1));
			/* 370: */}
		/* 371: */else
		/* 372: */{
			/* 373:387 */logger.info("isleave no");
			/* 374:388 */attendanceDate.setPresentDays(Integer
					.valueOf(attendanceDate.getPresentDays().intValue() + 1));
			/* 375: */}
		/* 376: */}
