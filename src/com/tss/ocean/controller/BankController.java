...
/* 30: */{
	/* 31: 40 */private static final Logger logger = Logger
	...
	/* 40: */public ModelAndView add_bank_get(
	...
	/* 42: */{
		...
		/* 49: 61 */if (error != null) {
			...
			/* 51: */}
		/* 52: 64 */mav.getModelMap().put("countryList",
				this.countryDAO.getList());
	...
	/* 59: */public ModelAndView add_bank_post(
			@ModelAttribute("purorder") @Valid Bank bank, BindingResult result,
	...
	/* 61: */{
		...
		/* 64: */{
			/* 65: 77 */int insertResult = ((Integer) this.bankDAO.insert(bank))
					.intValue();
			/* 66: 78 */if (insertResult > 0)
			/* 67: */{
				/* 68: 79 */System.out.println("Bank Added");
				/* 69: 80 */logger.log(Level.INFO,
						"Bank Added Successfully with id={0}",
						Integer.valueOf(insertResult));
				/* 70: 81 */return new ModelAndView("redirect:add-bank.html")
						.addObject("success", Utilities.getSpringMessage(
								this.messageSource, "bank.add.success", locale));
				/* 71: */}
			/* 72: 84 */logger.log(Level.WARNING,
					"Error occurred inserting bank:{0}", bank.toString());
			/* 73: 85 */model.put("countryList", this.countryDAO.getList());
			/* 74: 86 */return new ModelAndView("purchase_order", model)
					.addObject("error", Utilities.getSpringMessage(
							this.messageSource, "bank.add.error", locale));
			/* 75: */}
		/* 76: 90 */logger.log(Level.WARNING, "Bank values are not valid:",
				bank.toString());
	...
	/* 82: */public ModelAndView getBanks(
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error,
			Locale locale)
	/* 83: */throws Exception
	/* 84: */{
		...
		/* 92:107 */Map<Integer, String> countryMap = new HashMap(
				countryList.size());
	...
	/* 102: */public ModelAndView edit_bank_get(@RequestParam("id") int id,
			Locale locale,
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error)
	...
	/* 104: */{
		...
		/* 106: */
		/* 107:123 */Bank bank = (Bank) this.bankDAO
		...
		/* 110: */{
			...
			/* 113:127 */mav.getModelMap().put("countryList",
					this.countryDAO.getList());
			/* 114: */}
		/* 115: */else
		/* 116: */{
	...
	/* 129: */public ModelAndView edit_bank_post(
			@ModelAttribute("bank") @Valid Bank bank, BindingResult result,
	...
	/* 131: */{
		...
		/* 134: */{
			...
			/* 137: */{
				/* 138:149 */logger.log(Level.INFO,
						"bank updated Successfully with id={0}",
						Integer.valueOf(updateResult));
				/* 139:150 */return new ModelAndView("redirect:bank.html")
						.addObject("success", Utilities.getSpringMessage(
								this.messageSource, "purorder.update.success",
								locale));
				/* 140: */}
			/* 141:153 */logger.log(Level.WARNING,
					"Error occurred updating purchase_order:{0}",
					bank.toString());
			/* 142:154 */model.put("countryList", this.countryDAO.getList());
			/* 143:155 */return new ModelAndView("add_bank", model).addObject(
					"error", Utilities.getSpringMessage(this.messageSource,
							"purorder.update.error", locale));
			/* 144: */}
		/* 145:159 */logger.log(Level.WARNING,
				"Purchase_order values are not valid:", bank.toString());
	...
	/* 154: */{
		...
		/* 156:169 */Bank bank = (Bank) this.bankDAO
				.getRecordByPrimaryKey(Integer.valueOf(id));
		...
		/* 158: */{
			...
			/* 161: */{
				/* 162:173 */logger.log(Level.INFO,
						"Bank with id {0} deleted successfully", bank.getId());
				/* 163:174 */return true;
				/* 164: */}
			/* 165:176 */logger.log(Level.WARNING,
					"Error occurred while deleting bank with id {0}",
					bank.getId());
			/* 166:177 */return false;
			/* 167: */}
		/* 168:180 */logger.log(Level.INFO,
				"Bank with id {0} is already deleted", Integer.valueOf(id));
		/* 169:181 */return true;
		/* 170: */}
	/* 171: */
}
