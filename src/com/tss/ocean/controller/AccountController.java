...
/* 26: */{
	/* 27: 28 */private static final Logger logger = Logger
	...
	/* 35: */public ModelAndView add_account_get(
	...
	/* 54: */public ModelAndView add_account_post(
			@ModelAttribute("account") @Valid Accounts account,
	...
	/* 56: */{
		...
		/* 59: */{
			...
			/* 64: 67 */int insertResult = ((Integer) this.accountsDAO
					.insert(account)).intValue();
			/* 65: 68 */if (insertResult > 0)
			/* 66: */{
				/* 67: 69 */logger.log(Level.INFO,
						"Account Added Successfully with id={0}",
						Integer.valueOf(insertResult));
				/* 68: 70 */return new ModelAndView("redirect:add-account.html")
						.addObject("success", Utilities.getSpringMessage(
								this.messageSource, "account.add.success",
								locale));
				/* 69: */}
			/* 70: 74 */logger.log(Level.WARNING,
					"Error occurred inserting account:{0}", account.toString());
			/* 71: 75 */return new ModelAndView("account", model).addObject(
					"error", Utilities.getSpringMessage(this.messageSource,
							"account.add.error", locale));
			/* 72: */}
		/* 73: 80 */logger.log(Level.WARNING, "Account values are not valid:",
				account.toString());
	...
	/* 79: */public ModelAndView edit_account_get(@RequestParam("id") int id,
			@RequestParam(value = "success", required = false) String success,
			@RequestParam(value = "error", required = false) String error)
	/* 80: */throws Exception
	/* 81: */{
		...
		/* 83: */
		/* 84: 92 */Accounts account = (Accounts) this.accountsDAO
	...
	/* 106: */public ModelAndView edit_account_post(
			@ModelAttribute("account") @Valid Accounts account,
			BindingResult result,
			ModelMap model,
			@RequestParam(value = "imageUpload", required = false) MultipartFile upload,
	...
	/* 108: */{
		...
		/* 111: */{
			...
			/* 116: */else
			/* 117: */{
				/* 118:122 */Accounts oldAccount = (Accounts) this.accountsDAO
						.getRecordByPrimaryKey(account.getId());
			...
			/* 123: */{
				/* 124:127 */logger.log(Level.INFO,
						"Account updated Successfully with id={0}",
						Integer.valueOf(updateResult));
				/* 125:128 */return new ModelAndView("redirect:account.html")
						.addObject("success", Utilities.getSpringMessage(
								this.messageSource, "account.update.success",
								locale));
				/* 126: */}
			/* 127:132 */logger.log(Level.WARNING,
					"Error occurred updating account:{0}", account.toString());
			/* 128:133 */return new ModelAndView("account", model).addObject(
					"error", Utilities.getSpringMessage(this.messageSource,
							"account.update.error", locale));
			/* 129: */}
		/* 130:138 */logger.log(Level.WARNING, "Account values are not valid:",
				account.toString());
	...
	/* 157: */{
		...
		/* 159:165 */Accounts account = (Accounts) this.accountsDAO
				.getRecordByPrimaryKey(Integer.valueOf(id));
		...
		/* 161: */{
			...
			/* 165: */{
				/* 166:170 */logger.log(Level.INFO,
						"Account with id {0} deleted successfully",
						account.getId());
				/* 167:171 */return true;
				/* 168: */}
			/* 169:174 */logger.log(Level.WARNING,
					"Error occurred while deleting account with id {0}",
					account.getId());
			/* 170:175 */return false;
			/* 171: */}
		/* 172:178 */logger.log(Level.INFO,
				"Account with id {0} is already deleted", Integer.valueOf(id));
		/* 173:179 */return true;
		/* 174: */}
	/* 175: */
}
